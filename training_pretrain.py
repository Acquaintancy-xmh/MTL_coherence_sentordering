...
#
def train(model, pretrain_optimizer, optimizer, scheduler, dataset_train, dataset_valid, dataset_test, config, evaluator):  # valid_feed can be None
    ...
    # pretrain
    for cur_epoch in range(config.pretrain_epoch): 
        ...
        for text_inputs, label_y, *remains in dataloader_train:
            ...
            #     if "so" in name: print(name, value)
                # print(name, value.requires_grad)

            if scheduler is not None:
                # scheduler.step()
                scheduler.step(loss)
            pretrain_batch_cnt = pretrain_batch_cnt + 1

            # print train process
            if pretrain_batch_cnt % config.print_step == 0:
                logger.info("{}/{}-({:.3f})".format(pretrain_batch_cnt % config.ckpt_step, config.ckpt_step, loss))
            ...
            ##########
            ## validation
            if pretrain_batch_cnt % ckpt_step == 0:  # manual epoch printing
