...
class CorpusGCDC(CorpusBase):
    ...
    def generate_kfold(self, config, seed):
        ...
        cur_train_np = arr_train
        cur_test_np = arr_test
        ...
        for cur_fold in range(num_fold):
            ...
            pd.DataFrame(cur_train_np, columns=col_gcdc).to_csv(os.path.join(path_gcdc, path_fold_dir, cur_train_file), index=None)
            pd.DataFrame(cur_test_np, columns=col_gcdc).to_csv(os.path.join(path_gcdc, path_fold_dir, cur_test_file), index=None)
