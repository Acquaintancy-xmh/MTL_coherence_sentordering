...
class Model_DIS_Order(models.model_base.BaseModel):
    def __init__(self, config, corpus_target, embReader):
        ...
        ### Sentence Ordering Linear
        so_fc_in_size = self.base_encoder.encoder_out_size * 2
        so_linear_1_out = so_fc_in_size // 2
        so_linear_2_out = so_linear_1_out // 2

        self.so_linear_1 = nn.Linear(so_fc_in_size, so_linear_1_out)
        nn.init.xavier_uniform_(self.so_linear_1.weight)

        self.so_linear_2 = nn.Linear(so_linear_1_out, so_linear_2_out)
        nn.init.xavier_uniform_(self.so_linear_2.weight)

        self.so_linear_out = nn.Linear(so_linear_2_out, 1)
        nn.init.xavier_uniform_(self.so_linear_out.weight)
    ...
    def forward(self, text_inputs, mask_input, len_seq, len_sents, tid, len_para=None, list_rels=None, mode=""):
        ...
        encoded_sents = avg_sents_repr  # avg_sents_repr : [embedding, sent_num]
        # print(avg_sents_repr.shape)
        ...
        for batch_i in range(batch_size):
            ...
            if order_score_list.shape[0] == 0: order_score_list = order_score
            else:
                # print(order_label)
                # print(order_score_list)
                # print(order_score) 
        ...
        # return fc_out
        return outputs # [fc_out, order_label_list, order_score_list]
