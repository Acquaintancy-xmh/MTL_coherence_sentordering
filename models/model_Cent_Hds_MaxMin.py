...
class Coh_Model_Cent_Hds_MaxMin(models.model_base.BaseModel):
    def __init__(self, config, corpus_target, embReader):
        ...
        # self.so_linear_1 = nn.Linear(so_fc_in_size, 1)
    ...
    def get_fwrd_centers(self, text_inputs, mask_input, len_sents):
        ...
        for sent_i in range(self.max_num_sents):
            ...
            if cur_max_len > 0:
                ...
                attn_sent = cur_encoded[1]  # averaged attention for the current sent (batch, item, item)

                ## filter out: we do not consider special tokens and punctation as a center; <.>, <sep>, and <cls>
                list_diag = []
                for batch_ind, cur_mat in enumerate(attn_sent): # cur_mat:(item, item)
                    ...
                    if cur_batch_sent_len > 3:
                        cur_diag[cur_batch_sent_len-3:] = 0  # also remove puntation   # 因为XLNet的SEP CLS 在句子末尾
                    else:
